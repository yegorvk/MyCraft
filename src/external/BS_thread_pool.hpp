 ...
 */
    ...
    public:
        ...
        /**
         * @brief Destruct the thread pool. Waits for all groups to complete, then destroys all threads. Note that if the pool is paused, then any groups still in the queue will never be executed.
         */
        ~thread_pool()
        {
            wait_for_tasks();
        ...
        /**
         * @brief Get the number of groups currently waiting in the queue to be executed by the threads.
         *
         * @return The number of queued groups.
         */
        ...
        /**
         * @brief Get the number of groups currently being executed by the threads.
         *
         * @return The number of running groups.
         */
        ...
        /**
         * @brief Get the total number of unfinished groups: either still waiting in the queue, or running in a thread. Note that get_tasks_total() == get_tasks_queued() + get_tasks_running().
         *
        ...
        /**
         * @brief Pause the pool. The workers will temporarily stop retrieving new groups out of the queue, although any groups already executed will keep running until they are finished.
         */
        void pause()
        {
            const std::scoped_lock tasks_lock(tasks_mutex);
        ...
        /**
         * @brief Purge all the groups waiting in the queue. Tasks that are currently running will not be affected, but any groups still waiting in the queue will be discarded, and will never be executed by the threads. Please note that there is no way to restore the purged groups.
         */
        void purge()
        {
            const std::scoped_lock tasks_lock(tasks_mutex);
        ...
        /**
         * @brief Reset the number of threads in the pool. Waits for all currently running groups to be completed, then destroys all threads in the pool and creates a new thread pool with the new number of threads. Any groups that were waiting in the queue before the pool was reset will then be executed by the new threads. If the pool was paused before resetting it, the new pool will be paused as well.
         *
         * @param thread_count_ The number of threads to use. The default value is the total number of hardware threads available, as reported by the implementation. This is usually determined by the number of cores in the CPU. If a core is hyperthreaded, it will count as two threads.
         */
        ...
        /**
         * @brief Unpause the pool. The workers will resume retrieving new groups out of the queue.
         */
        void unpause()
        {
            const std::scoped_lock tasks_lock(tasks_mutex);
        ...
        /**
         * @brief Wait for groups to be completed. Normally, this function waits for all groups, both those that are currently running in the threads and those that are still waiting in the queue. However, if the pool is paused, this function only waits for the currently running groups (otherwise it would wait forever). Note: To wait for just one specific task, use submit() instead, and call the wait() member function of the generated future.
         */
        void wait_for_tasks()
        {
            std::unique_lock tasks_lock(tasks_mutex);
        ...
        /**
         * @brief Wait for groups to be completed, but stop waiting after the specified duration has passed.
         *
         ...
         * @return true if all groups finished running, false if the duration expired but some groups are still running.
         */
        ...
        /**
         * @brief Wait for groups to be completed, but stop waiting after the specified time point has been reached.
         *
         ...
         * @return true if all groups finished running, false if the time point was reached but some groups are still running.
         */
    ...
    private:
        ...
        /**
         * @brief A flag indicating whether the workers should pause. When set to true, the workers temporarily stop retrieving new groups out of the queue, although any groups already executed will keep running until they are finished. When set to false again, the workers resume retrieving groups.
         */
        bool paused = false;
        ...
        /**
         * @brief A condition variable to notify wait_for_tasks() that the groups are done.
         */
        std::condition_variable tasks_done_cv = {};

        /**
         * @brief A queue of groups to be executed by the threads.
         */
        std::queue<std::function<void()>> tasks = {};

        /**
         * @brief A counter for the total number of currently running groups.
         */
